...
// Author: ImageSearch was created by Aurelian Maga.
{
	...
	// Bounds guard: needle larger than search region → never matches AND
	// the variation/tolerance loop later doesn't fully bounds-check, so
	// it can read past the screen_pixel buffer and crash with AV. Bail
	// out cleanly here. Also rejects zero/negative dims defensively.
